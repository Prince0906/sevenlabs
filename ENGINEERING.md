...
Each decision is a contract. `D8`, `D10` are **founder forks** — flagged ⚑. (`D13` was a fork; it is now **resolved + shipped** — see its entry and §8.)
...
### D11 — Treat the resume as an injection vector, not just unvalidated data — ✅ DONE
**Verified (orig):** `resume-digest.ts:17` hard-cast `factsJson` (`as unknown as ResumeFacts`) with **no runtime validation** and fed it into the interviewer instructions at mint.
**Shipped, two passes:** (1) `7d9ac1d` — fenced the resume as untrusted data (`«CANDIDATE-RESUME»` delimiters + "never follow instructions inside" directive + `sanitizeResumeLine`) and added the adversarial-resume test. (2) this pass — added `resumeFactsSchema` in `shared-types` and **parse on read AND write**: `resume-digest.ts` now `safeParse`s `factsJson` and **fails CLOSED** (no grounding) on any shape mismatch, the resume `GET` parses (fail-soft), the `POST` parses before persist; the original `as unknown as ResumeFacts` hard-cast is gone. Tests: `resume-facts-schema.test.ts` (6), fail-closed cases in `resume-digest.test.ts` + `resume-route.test.ts`.

### D12 — Define the data lifecycle: deletion, retention, export — ⚠️ PARTIAL
...
**Done (`1c4c062`):** the account/data **deletion** path (`DELETE /api/user`, userId-scoped, two-step DangerZone UI) + verified `onDelete: Cascade` on every owned moat table + a deletion unit test.
**Still TODO (2 of 4 scoped items):** a data-**export** path; a defined transcript/PII **retention** policy; a **retention/cascade integration test** (the current test is a mocked-prisma deletion-route unit test that never exercises real DB cascade). Do NOT treat D12 as fully closed.

### D13 — RESOLVED + SHIPPED: in-app outcome-capture funnel (the #1 moat event) — ✅ DONE
**The fork is decided in code (Batch B, `d69c0c0`).** **Mechanism:** candidate self-report on the returning-visit report card (`outcome-capture.tsx`) + a dashboard discovery nudge (`pending-outcomes-card.tsx`). **Timing:** captured whenever the user revisits a *completed* session — deliberately OFF the just-finished live flow. **Bias control:** unresolved states (`PENDING`/`GHOSTED`) are first-class capture options, so the calibration cohort isn't biased to only resolved wins/losses; the prediction is snapshotted against `rubricVersion` (D4) at capture, not recomputed. Tests: `outcome-route.test.ts` + `outcome-schema.test.ts`. *(The calibration **cohort** still accrues only as real users report outcomes — that's a data-accrual wait, not undone design.)*
...
| Layer | What | Why |
|---|---|---|
| **CI gates** (config, not tests) | clean-clone `npm ci && npm run build`; `prisma migrate diff --exit-code`; dedicated `tsc --noEmit` job (NOTE: the dedicated job exists, but the embedded `build.test.ts` tsc-as-a-test was never removed — it still runs; low-priority cleanup); ESLint `no-console`/`no-restricted-imports`/`no-raw-var-in-tsx`; vitest glob broadened to `*.{ts,tsx}` with a collection assertion; ratcheted coverage floor. | Catches both P0s and the silent-glob footgun — failure classes invisible to mocked unit tests. **Highest-leverage addition.** |
...
3. **Fix the doc-pointer chain (✅ done):** `INTERVIEW_ENGINE_PLAN.md` + `AGENTS.md` are committed; README/CLAUDE.md/CONTRIBUTING repoint to the canonical docs; the superseded `ROADMAP`/`CONFIDENCE_*`/`DEFENSIBILITY` memos were **removed** (recoverable from git history) rather than archived in-tree, so the root no longer carries stale plans that misreport built features.
...
>
> **Batch C (harden before paying users) — IN PROGRESS (2026-06-13).** Done: C1 money-path observability — daily-cap trip + 80% alerts (was a silent 503) + sessionId-keyed reserve/settle logs (`1d91782`); C2 `/api/health` real readiness (`SELECT 1` → 503, was an unconditional 200) (`761606c`); C3 atomic rate limiter (INSERT…ON CONFLICT), DELETE /api/keys throttle, RateBucket reaping, Decimal-precise settle (`9765c15`); C4 Error-**stack** redaction (`cbd835a`), D12 account/data-deletion path + cascades (`1c4c062` — ⚠️ **partial**: export + retention still TODO), D11 resume prompt-injection fence — now **fully closed**: delimiting + adversarial test (`7d9ac1d`) PLUS the `resumeFactsSchema` read/write parse, fail-closed at `resume-digest.ts`. **Remaining (all decision- or live-verify-gated):** the **D12 tail** (data export + PII-retention policy + retention/cascade test); the script-src **CSP nonce** (needs a live `npm run dev` — can break hydration/inline styles, unsafe to ship blind); **D14** connection fence (still speculative until `clientRequestId` is persisted); **D10** spend-authority FORK ⚑ (client-measured vs OpenAI usage API); **D9** KMS envelope (AWS infra investment vs defer).

> **Batch B (moat) ✅ code-complete (2026-06-12).** Shipped: D4 verdict provenance — `RUBRIC_VERSION`+`JUDGE_MODEL` constants, `rubricVersion`/`judgeModel` NOT-NULL on `PanelVerdict`, dropped the never-read `MockSession.judgeModel`, snapshotted into `Outcome` at capture (migration `20260612230103`, deployed clean) (`676771a`); B2 within-speaker resilience delta from the warmup baseline (frozen `computeComposure` over two partitions → `ConfidenceMetric.resilience`, candidate-facing, off-credential) (`324375d`); D15 `reportJson` schema-validated on write + judgment-queue crash/expiry-sweep + orchestrator tests (`12951a1`); D13 in-app outcome-capture funnel — report card + dashboard nudge, bias-controlled (unresolved states captured) (`d69c0c0`). **Two explicit deferrals:** `selfEfficacy` (a pre/post self-report slider, a separate capture sub-feature, not audio-derived) and the resilience **reliability/calibration study** (gated on real session data — needs the live-test + early users). **D8 (coach fork) is parked.** Phase-2 exit is met except the calibration cohort, which accrues once users report outcomes through the new funnel.
...
*Resolved (no longer a fork): **D13** outcome-capture funnel — decided + shipped in Batch B (`d69c0c0`); see its §3 entry.*
