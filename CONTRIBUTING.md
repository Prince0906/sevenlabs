...
- **Naming** ([ADR-0014](docs/decisions/0014-ubiquitous-language.md)): *interview* = the product surface (`/interview`, `/api/interview`, `src/features/interview/`, `src/lib/interview/`) · *panel* = the 3-interviewer committee mechanism (`packages/panel-core`, `panel-machine`, `panel-orchestrator`) · *providers* = vendor clients (`src/lib/providers/`). Never reintroduce `mock`/`coach` naming in code paths. DB identifiers (`MockSession`, `PracticeTurnRole.COACH`) are frozen legacy names — don't rename them outside a coordinated migration.
