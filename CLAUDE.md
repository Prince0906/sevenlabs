...
- **Turns**: `MockTurn` is single-writer, `seq`-ordered (`@@unique([sessionId, seq])`, seq assigned at dequeue). `role` reuses `PracticeTurnRole`; `role=COACH` means an interviewer seat (ADR-0013). The judge scores by `orderBy seq asc` — dropping/duplicating a COACH turn corrupts the verdict.
...
- **Disfluency / fluency**: `DEEPGRAM_API_KEY` set ⇒ verbatim ASR (`src/lib/providers/deepgram.ts`, `filler_words:true`) measures fillers/repeats/false-starts into `MockTurn.disfluencyJson`; unset ⇒ Whisper fallback, which cleans speech so disfluency reads artificially low (`disfluencyJson` null).
...
Highest-stakes invariants: every interview query is **userId-scoped**; the **judge stays on the house key** (pinned model); BYOK keys are **never echoed back**; **`turn_detection` stays `null`** (push-to-talk); `MockTurn` is **single-writer seq-ordered**. Each has an owning test (`docs/testing.md` §6).
...
Models by domain — **DB names are FROZEN legacy naming** (ADR-0014): `MockSession`/`MockTurn`/`MockStatus` = interview session/turn/status; `PracticeTurnRole.COACH` = interviewer seat. Don't rename outside a coordinated migration.
- **Auth.js**: `User` (tenant root; passwordHash nullable for Google-only users; carries `targetCompanies String[]`, `interviewDate?`, `targetLevel SignalLevel`), `Account`, `Session`, `VerificationToken`.
- **Interview**: `Scenario`, `PanelSeat` (`@@unique([scenarioId, seatOrder])`), `MockSession` (central run — `@@unique([userId, clientRequestId])` idempotency; `keySource`/`provider`/`apiKeyId`→ProviderKey/`spendCents`/`reportJson`), `MockTurn` (`@@unique([sessionId, seq])`, `clientTurnId`, `disfluencyJson`, `transcriptionMissing`), `DimensionScore`, `PanelVerdict` (`sessionId @unique`, `barRaiserVeto`, `rubricVersion`+`judgeModel`), `ConfidenceMetric`, `DrillAssignment`, `JudgmentJob` (PK=sessionId; lease queue, index `(status, leaseUntil)`).
...
9 migrations under `prisma/migrations/` (lock `provider = postgresql`); the latest, `20260711094329_remove_speaking_coach`, dropped the coach tables + `PracticeSessionStatus`, keeping `PracticeTurnRole`.
...
- **Naming** (ADR-0014): *interview* = product surface, *panel* = the committee mechanism, *providers* = vendor clients. Never reintroduce `mock`/`coach` in code paths; DB names stay frozen.
